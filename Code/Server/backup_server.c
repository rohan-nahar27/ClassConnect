...
int main(int argc, char * argv[])
{
	...
	(void) pthread_mutex_init(&sCredential.sC_mutex, 0);
...
void load_studentinfo()
{
    int size=0;
    char password[5120];
    pthread_mutex_t filecount_mutex;
    (void) pthread_mutex_init(&filecount_mutex, 0);
...
//printf("%d \t %s", sLogin.sId[size],sLogin.pwd[size]);
        size++;
	(void) pthread_mutex_unlock(&sLogin.sL_mutex);
    }
	(void) pthread_mutex_lock(&filecount_mutex);
    f_Count=size;
	(void) pthread_mutex_unlock(&filecount_mutex);
}
